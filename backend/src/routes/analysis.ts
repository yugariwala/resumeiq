...
analysisRouter.get('/analyses', validateJWT, async (req: AuthRequest, res: Response) => {
    try {
        ...
        const formatted = await Promise.all((analyses || []).map(async (a: any) => {
...
analysisRouter.get('/analysis/:id', validateJWT, async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params
...
analysisRouter.post('/analysis/:id/job-match', validateJWT, async (req: AuthRequest, res: Response) => {
    try {
        const { id } = req.params
